...
use openai_api_client::{Usage, ClientError};
...
impl ChatCompletionsParams {
    pub fn new( messages: Vec<HashMap<String,String>>) -> Self { Self {  messages,..Default::default() } }
}
...
pub async fn chat_completions(  prompt: &str, 
...
Ok(completions_response.choices[0].message.remove("content").unwrap_or("".to_string()))
    }